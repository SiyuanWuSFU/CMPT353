...
def date_to_month(d):
    ...
    return pd.to_datetime(d).dt.to_period('M')


def pivot_months_pandas(data):
    m = date_to_month(data['date'])
    data['months'] = m

    aggregated = data.groupby(["name","months"]).sum().reset_index()
    monthly = aggregated.pivot(index='name', columns='months', values='precipitation')

    c_aggregated = data.groupby(["name","months"]).count().reset_index()
    counts = c_aggregated.pivot(index='name', columns='months', values='station')
    #print(counts)
...
def main():
    ...
    totals.to_csv('totalss.csv')
    counts.to_csv('countss.csv')
